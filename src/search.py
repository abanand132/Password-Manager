...
#                     decrypted_email_data = cipher.decrypt(text1)
#                     decrypted_password_data = cipher.decrypt(text2)
#                     email.delete(0, END)
...
def search_func(pin_entry, pin_value, website, search_win, delete_data_btn):
    website_data = website.get().lower().strip()

    # Sending to decryption function present in search.py
    ...
    if pin_entry.get() == pin_value and len(website_data) != 0:
        ...
        try:  # reading json file for searching purpose. searching logic below
            ...
            if len(result) == 0:
                ...
                website.insert(0, result[0][0])
                email_search.insert(0, cipher.decrypt(result[0][1]))
                password_search.insert(0, cipher.decrypt(result[0][2]))
                ...
                if messagebox.askyesno(title="Search Result",
                    ...
                    multiple_result.multiple_result_func(result)
