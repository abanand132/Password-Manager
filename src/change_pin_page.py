...
def change_pin_func(window, search_win):
    ...
    def send_code_func():
        ...
        if email_entry.get() == cipher.decrypt(loading_data['email']):
            pin = send_email()
            if pin:
    ...
    def submit_btn_func():
        ...
        if code_save_in_file == code_sent_to_mail and code_save_in_file != "":
    ...
    def modification_func():
        ...
        def save_pin_func():
            ...
            loading_data['pin'] = cipher.encrypt(new_pin)
            loading_data['code'] = ""
            with open('basic_info.json', 'w') as data:  # updating code in json file
                json.dump(loading_data, data, indent=4)
