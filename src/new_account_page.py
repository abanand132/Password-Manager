...
def create_account_func(home_window):
    ...
    if len(data['email']) > 0:
        ...
        var.set(0)  # it states that now save btn will point it to update_func else it points to save_func
    ...
    def save_func():
        with open("basic_info.json", "w") as file:
            if len(email.get()) != 0 and len(pin.get()) != 0:
                user_data = {
                        "email": cipher.encrypt(email.get()),
                        "pin": cipher.encrypt(pin.get()),
                        "theme": "light",
                        "code": ""
                    }
                email_page.welcome_mail(email.get())  # sending welcoming mail to the user
    ...
    def update_func():
        ...
        if len(email.get()) > 0 and cipher.decrypt(load_data['email']) == email.get():
            save_button.destroy()
            email_label.config(text='New Email : ')
            email.delete(0, END)
            code_sent_to_email = email_page.send_email()  # calling email function which sent code

            load_data['code'] = cipher.encrypt(code_sent_to_email)  # encrypting code that is sent
            ...
            def submit_btn_func():
                ...
                if code_entry.get() == cipher.decrypt(code_save_in_file):
                    load_data1['email'] = cipher.encrypt(email.get())
                    load_data1['code'] = ""
